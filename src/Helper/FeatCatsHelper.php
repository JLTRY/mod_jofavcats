				...
				for ($j=$start;$j<($start+$count+$subcount);$j++)
				{
					if (isset($items[$j])) {
						...
						if ( $j < $count+$start ) 
						{
							...
							if ($show_date) {
								$items[$j]->displayDate = HTMLHelper::_('date', $items[$j]->$show_date_field, $show_date_format);
							}
							if ($show_author) {
							...
							if ($show_image) :
								...
								if (!$img && $yt_image != 2) :
									...
									elseif (isset($images->image_fulltext) and !empty($images->image_fulltext)) :
										...
										$regex   = "/<img[^>]+src\s*=\s*[\"']\/?([^\"']+)[\"'][^>]*\>/";
										$img = self::srch_first_occ($regex,$content);
									endif;
								endif;
								...
								if ($img or $def_image) {
									if ($params->get('thumb_image', 1)) {
										...
										$img_base = !self::is_absolute($img)?URI::base(false):'';
	...
	public static function getThumbnail($img,&$params,$item_id,$alt) 
	{
		...
		if(!$size) 
		{
			...
			if(file_exists($thumb_path)){
				...
				self::calculateSize($origw, $origh, $width, $height, $proportion, $newwidth, $newheight, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h);
